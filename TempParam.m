...
GFm = RFInterp(pCO2,'CO2',v);       % W/m2; modern GHF at pCO2 = 400 ppm
...
switch method
    case 'A'
        GFa = RFInterp(100*pCO2,'CO2',v); % W/m2; archean GHF at pCO2 = 30,000 ppm
    otherwise
...
%% Calculate the constants (aT, bT) that equate the greenhouse forcing for the two eras
...
% re-enter into first equation to find a1 for modern Earth (should == Archean)
    % yir_m = a1Ts + b1GFm
a1 = aconstant(yir_m,b1,GFm,Ts,v.atm.Tx);% thermal IR optical depth Temp constant
...
%% calculate the pressure broadening parameter q 
...
%% If we want to include aerosol haze, calculate y_hz constants
...
% % based on the above figure, we want to assume haze behaviour much closer
% % to that observed on Titan in order to set our y_hz parameterization
% % this means that we will assume that formed hazes are not strongly opaque
% % in the visible, but only moderately (so dT is not >15 K under haze!)
% yhzm = 1-exp(-v.atm.hz.beta.*v.atm.hz.g.*yir_m); % where g = 0.28
% 
% but we still want to relate this to CH4:CO2, so find the constants anyway!
    % y_hz = a2(CH4/CO2) 
% therefore:
    % a2 = y_hz./(CH4/CO2)
% a2 = 1./0.1; % tuned so that at CH4:CO2 = 0.1, y_hz = 1 (ie. totally visibly opaque)
% or just lightly opaque, y_hz = 0.5 (max)
a2 = 0.5./0.1; 

% see what happens just by varying a2 for CH4/CO2 = 0.1
switch PLOT
    case 'on'
        a2list = 0.1:0.5:10; 
        figure(); clf; 
        for ia = 1:length(a2list)
           yhza(ia) = a2list(ia).*0.1;  
        end
        plot(a2list,yhza); ylabel('y_{hz} param at CH4/CO2 = 0.1'); xlabel('a_2 param'); 
end
...
tp.CS  = climatesensitivity(pCO2,tp.aT,tp.bT,v);    % find climate sensitivity (dK per CO2 doubling, 2.5-4
...
function acon = aconstant(yir,bcon,RF,T,Tx)
% simplified to exclude pressure broadening (Y == gamma)
    % y_ir = Y(a1 Ts + b1 RF) = a1 Ts + b1 RF
    % a1 = (y_ir - b1 RF) / Ts
    acon = (yir - (bcon.*RF))./ (T-Tx); % relation constant
end
...
%     
q = log(yir2pan ./ (acon.*(T2pan - v.atm.Tx) + bcon.*RF))./log(2); 
...
function CS = climatesensitivity(CO2,a,b,v)
% With our prototype parametrization, find the climate sensitivity by doubling CO2

GF_0 = RFInterp(CO2,'CO2',v) ;           % greenhouse forcing modern CO2
GF_d = RFInterp(2.*CO2,'CO2',v) ;        % greenhouse forcing with doubled CO2
Teq_0 = (((((v.S.Pref/4).*(1-v.ea.alb))+(v.const.aRF.*GF_0.^v.const.bRF))./v.const.bol).^(1/4)); % equilibrium temp
Teq_d = (((((v.S.Pref/4).*(1-v.ea.alb))+(v.const.aRF.*GF_d.^v.const.bRF))./v.const.bol).^(1/4)); % equilibrium temp with double CO2
T_0 = ( (((1-v.ea.alb)*v.S.Pref/4) .* (1 + (3/4).* (a.*(Teq_0 - v.atm.Tx) + b.*GF_0) )) ./ v.const.bol).^(1/4) ; %
T_d = ( (((1-v.ea.alb)*v.S.Pref/4) .* (1 + (3/4).* (a.*(Teq_d - v.atm.Tx) + b.*GF_d) )) ./ v.const.bol).^(1/4) ; %
